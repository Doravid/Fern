...
// Windows Performance issues. At least on my windows laptop it runs horribly vs my Linux (Pop_OS) box.
...
// Modifed hierarchy function, modified to render shadows.
...
function render() {
    ...
    //Move the camera / car if they are supposed to move.
...
function renderFace(face, isShadow = false) {
    ...
    //Pass in if we are using reflections / refractions. 
    let isCar = (currentModel === models[3]);
    let isBunny = (currentModel === models[4]);

    gl.uniform1i(reflectionLoc, enableReflections && isCar);
